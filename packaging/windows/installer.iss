...
Filename: "powershell.exe"; Parameters: "-NoProfile -NonInteractive -ExecutionPolicy Bypass -Command ""\
    $eth = Get-NetAdapter | Where-Object { $_.PhysicalMediaType -eq '802.3' -and $_.Status -eq 'Up' } | Sort-Object InterfaceIndex | Select-Object -First 1; \
    if ($eth) { \
        $existing = Get-NetIPAddress -InterfaceIndex $eth.InterfaceIndex -AddressFamily IPv4 -ErrorAction SilentlyContinue | Where-Object { $_.IPAddress -notlike '169.254.*' -and $_.IPAddress -ne '127.0.0.1' }; \
        if (-not $existing) { \
            Remove-NetIPAddress -InterfaceIndex $eth.InterfaceIndex -AddressFamily IPv4 -Confirm:$false -ErrorAction SilentlyContinue; \
            New-NetIPAddress -InterfaceIndex $eth.InterfaceIndex -IPAddress '169.254.200.1' -PrefixLength 16 -ErrorAction SilentlyContinue; \
            Set-NetIPInterface -InterfaceIndex $eth.InterfaceIndex -AddressFamily IPv4 -DadTransmits 0 -ErrorAction SilentlyContinue; \
            Write-Host 'DJ Link NIC configured: 169.254.200.1/16 DAD=0' \
        } else { \
            Write-Host ('DJ Link NIC already has IP: ' + ($existing | Select-Object -First 1 -ExpandProperty IPAddress)) \
        } \
    } else { \
        Write-Host 'No Ethernet adapter found — skipping NIC config' \
    }"""; \
    Flags: runhidden
